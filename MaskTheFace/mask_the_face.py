...
parser.add_argument(
    '--output_dir',
    default="",
    help="Output directory for writing masked faces"
)
...
# Process if the path was a file
    ...
    write_path = args.path.split(".")[0]
    file_name = os.path.basename(args.path)

    file_initial, file_extension = file_name.split('.')[0], file_name.split('.')[1]
    file_name = file_initial + '_' + args.mask_type + '.' + file_extension
    ...
    if is_image(image_path):
        ...
        for i in range(len(mask)):
            w_path = os.path.join(write_path,file_name)
            print(w_path)
            img = masked_image[i]
            cv2.imwrite(w_path, img)
else:
